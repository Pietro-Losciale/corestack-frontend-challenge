...
form.addEventListener('submit', function(e) {
  ...
  // elementi messaggi UI
  const errorMessage = document.getElementById('errorMessage');
  const successMessage = document.getElementById('successMessage');
  const emptyMessage = document.getElementById('emptyMessage');

  // gestione input vuoti- upgrade con alert al posto del popup del browser
  if (!title || !body) {
  ...
  // logica per evitare la creazione di post con titoli duplicati
  ...
  if (exists) {
    ...
    // nasconde eventuale messaggio di successo
